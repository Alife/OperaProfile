...
(function(){
   ...
   + '|^http://[0-9a-z-]+.gstatic.com/|^http://[0-9a-z-]+.appspot.com/|^http://yui.yahooapis.com/|^http://script.aculo.us/|^http://ipinfodb.com/|^http://api.recaptcha.net/'
   ...
   if(enabled)window.opera.addEventListener('BeforeExternalScript', function(e){
      ...
      k2 = k[2];
      //alert(k);
      var j = window.location.hostname.split('.',2);
      j = j[1];
      if(k1.indexOf(j)!=-1 || k2.indexOf(j)!=-1 || j.indexOf(k1)!=-1|| j.indexOf(k2)!=-1)return;
      var h = window.location.hostname;
      var n = !/\.(com|[a-z]{2})$/.test(h);
      var a = s.match(/^https?:\/\/([^\/]+@)?([^:\/]+)/i);
      var d = a ? a[2] : h;
