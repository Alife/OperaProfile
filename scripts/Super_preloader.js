...
// @author NLF
// @description  预读+翻页..全加速你的浏览体验...#^_^#...(Support Opera 10.1+ ,Fx3.6+(need GreaseMonkey) ,Chrome7.0+)..
// @create 2010-4-30
...
(function(){
		...
		if(browser.opera){
			...
			*/
			document.addEventListener('DOMContentLoaded',function(){
				...
				window.parent.postMessage('NX_preiframe:DOMLoaded','*');
			},false);
		}else{
			//window.parent.postMessage('NX_preiframe:DOMLoaded','*');//iframe里面 chrome无法访问window.parent,返回undefined
	...
	function init(browser,window,document,db,G_window,opera,scriptStorage){
		...
		(function(prefs,opera,scriptStorage,updateSet){
			...
			if(opera){
				var crossMessage=function(url,loadFN){
					...
					window.addEventListener('message',function(e){
						var data=e.data,origin=e.origin;//发送消息的框架所在的域名
						//alert(data);
		...
		//获取单个元素,混合
		function getElement(selector,doc,win){
			var ret;
			...
			if(typeof selector=='string'){
				if(selector.search(/^css;/i)==0){
					ret=getElementByCSS(selector.slice(4),doc);
				}else if(selector.toLowerCase()=='auto;'){
					ret=autoGetLink(doc,win);
				}else{
					ret=getElementByXpath(selector,false,doc);
				};
			}else{
				ret=selector(doc,win);
			};
			return ret;
		};

		//获取多个元素
		function getAllElements(selector,doc,win){
			var ret=[];
			if(!selector)return ret;
			var Eles;
			doc=doc || document;
			...
			if(typeof selector=='string'){
				if(selector.search(/^css;/i)==0){
					Eles=getAllElementsByCSS(selector.slice(4),doc);
				}else{
					Eles=getAllElementsByXpath(selector,false,doc);
				};
			}else{
				Eles=selector(doc,win);
			};

			var pro=Eles.snapshotLength===undefined? 'length' : 'snapshotLength';
			var i=Eles[pro]-1;
			if(pro=='snapshotLength'){//快照
				for(;i>=0;i--){
					ret.unshift(Eles.snapshotItem(i));
					//ret[i]=Eles.snapshotItem(i);
				};
			}else{
				for(;i>=0;i--){//类数组
					ret.unshift(Eles[i]);
		...
		if(window.localStorage){
			...
			var getCookie=function(c_name){
				...
				var value=(document.cookie.match(ore) || ['',''])[1];
		...
		function getFullHref(href){
			...
			//alert(href);
			if(href.search(/^https?:/)==0)return href;
			var a=getFullHref.a;
			if(!a){
	...
	//动画库
		...
		function floatWindow(){
							...
							<ul>\
								<li>\
									...
									<ul id="sp-fw-a_useiframe-extend">\
										<li>\
											<input type="checkbox" title="等待iframe完全载入后(发生load事件),将内容取出,否则在DOM完成后,就直接取出来..(勾上后,可能会比较慢,非opera强制勾上,不勾上有问题)" id="sp-fw-a_iloaded" name="sp-fw-a_iloaded" />等待iframe完全载入\
										</li>\
										<li>\
											<input type="number" title="在可以从iframe取数据的时候,继续等待设定的毫秒才开始取出数据(此项为特殊网页准备,如果正常,请设置为0)" id="sp-fw-a_itimeout" name="sp-fw-a_itimeout"/>ms延时取出\
										</li>\
									</ul>\
								</li>\
								<li>\
			...
			savebutton.addEventListener('click',function(e){
				...
				if(SSS.a_enable!==undefined){
					...
					value.a_itimeout=isNaN(t_a_itimeout)? SSS.a_itimeout : t_a_itimeout;
					var t_a_remain=Number(gl(a_remain));
					value.a_remain=isNaN(t_a_remain)? SSS.a_remain : (t_a_remain>=1? t_a_remain : Number(t_a_remain.toFixed(2)));
					var t_a_maxpage=Number(gl(a_maxpage));
					value.a_maxpage=isNaN(t_a_maxpage)? SSS.a_maxpage : t_a_maxpage;
					var t_a_ipages_1=Number(gl(a_ipages_1));
					value.a_ipages=[gl(a_ipages_0),(isNaN(t_a_ipages_1)? SSS.a_ipages[1] : t_a_ipages_1)];
		...
		function autopager(SSS){
					...
					if(SSS.a_iloaded){
						...
						if(browser.opera){
							function messagehandler(e){
								if(!messageR && e.data=='NX_preiframe:DOMLoaded'){
									//alert(e.source);
									messageR=true;
									iframeLoaded.call(i);
								};
							...
							setInterval(function(){
								if(!messageR && i.contentDocument.body.hasAttribute('data-iframe-loaded')){
									messageR=true;
									i.contentDocument.body.removeAttribute('data-iframe-loaded');
									iframeLoaded.call(i);
								};
							},100);
						};
			...
			function insertedIntoDoc(type){ // default type=next
				...
				var pageElement=getAllElements(SSS.a_pageElement,doc,win);
				var ii=pageElement.length;
				var i,pe_x,pe_x_nn,scripts,scripts_x,j;
				if(ii>0){
					for(i=0;i<ii;i++){
						pe_x=pageElement[i];
						...
						scipts=pe_x.getElementsByTagName('script');
						for(j=scipts.length-1;j>=0;j--){//移除脚本
							scipts_x=scipts[j];
					...
					if(SSS.a_replaceE){
						...
						//alert(oldE_lt);
						if(oldE_lt>0){
							var newE=getAllElements(SSS.a_replaceE,doc,win);
					...
					//alert(SSS.nextLink)
					var nl=getElement(SSS.nextLink,doc,win);
					//alert(nl);
					if(nl){
						nl=getFullHref(nl);
						if(nl==nextlink){
		...
		function autoGetLink(doc,win){
			...
			if(firstCheck){//只在第一次检测的时候,抛出上一页链接.
				prelink=_prelink;
				firstCheck=false;
			};
		...
		if(!browser.opera){//非opera浏览器iframe翻页DOMloaded后取出会导致图片显示不完成.
			SSS.a_iloaded=true;
			//SSS.a_iloaded=false;
		};
		if(!SSS.hasRule){
