...
(function(){
	...
	var autoMatch={
		...
		pfwordl:{//关键字前面的字符限定.
			previous:{//上一页关键字前面的字符,例如 "上一页" 要匹配 "[上一页" ,那么prefix要的设置要不小于1,并且character要包含字符 "["
				enable:true,
				maxPrefix:3,
				character:[' ','　','[','［','<','＜','‹','«','<<','『','「','【','(','←','上','上一','上1','翻上','前','新','较新']
			},
			next:{//下一页关键字前面的字符
				enable:true,
				maxPrefix:2,
				character:[' ','　','[','［','『','「','【','(','←','← ','下','下一','下1','翻下','后','後','舊','早期','次']
			}
		},
		sfwordl:{//关键字后面的字符限定.
	...
	//上一页关键字	//下一页关键字
	var prePageKey= ['张','張','幅','章','节','節','篇','页','頁','后退','往前','文章','へ','<','→','previous','previous Page','Older','後退'];
	var nextPageKey=['张','張','幅','章','节','節','篇','页','頁','前进','往后','文章','へ','>','←','Next'    ,'Next Page'    ,'Newer','翻页','翻頁'];
