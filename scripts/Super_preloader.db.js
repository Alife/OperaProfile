...
(function(){
	...
	//上一页关键字	//下一页关键字
	var prePageKey= ['上一页','上一頁','上1页','上1頁','上页','上頁','翻上頁','翻上页','上一张','上一張','上一幅','上一章','上一节','上一節','上一篇','前一页','前一頁','后退','上篇','前页','往前','舊文章','前へ','<','previous','previous Page','Older','後退'];
	var nextPageKey=['下一页','下一頁','下1页','下1頁','下页','下頁','翻下頁','翻下页','下一张','下一張','下一幅','下一章','下一节','下一節','下一篇','后一页','後一頁','前进','下篇','后页','往后','新文章','次へ','>','Next'    ,'Next Page'    ,'Newer','翻页','翻頁'	];
	...
	var SITEINFO=[
		...
		{siteName:'火影忍者中文网',
			...
			nextLink:function(D,W){
				if(!W.location)return;
				var s2=D.getElementById('topSelect');
				if(s2){
					var s2os=s2.options
					var s2osl=s2os.length;
					//alert(s2.selectedIndex);
	...
	var SITEINFO_TP=[
		{siteName:'Discuz论坛帖子列表页面',
			...
			autopager:{
				useiframe:true,
					iloaded:false,
				pageElement:'//form[@method="post"][@name] | //div[@id="postlist"] | //div[@class="tl"] | //div[@class="vt"]',
			}
		},
