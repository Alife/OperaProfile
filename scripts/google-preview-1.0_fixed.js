...
//    var preview = "http://"+getGPSub(idnasite)+".googlepreview.com/preview?s=" + protocol + idnasite;
    var preview = "http://"+getGPSub(idnasite)+".searchpreview.de/preview?s=" + protocol + idnasite;
//c.searchpreview.de/preview?s=https://chrome.google.com&ra=1
...
{
  fd =getFullDomain(href);
  if (fd.match(/(.*wrs|\.rds|rds)\.yahoo\.com/i)) {
  ...
  else if (fd.match(/(.*wrs|\.rds|rds)\.search\.yahoo\.co\.jp/i)) {
    var hhref = href.match(/\*-.+$/);
    if (hhref) {
      href = unescape(hhref[0].substr(2));
    }
  }
    return href;
}
...
//    return;
// 注释：为了配合oAutoPagerize 对自动拼接的页面也添加截图
...
function isYahoo(href) {
  return href.match(/http:\/\/.*search\.yahoo\..+\/search/i);
}
...
window.addEventListener("scroll", function() { GP_main(false); }, true);
